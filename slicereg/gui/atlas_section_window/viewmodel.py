...
class AtlasSectionViewModel(HasObservableAttributes):
    ...
    def _update_section_position(self, x, y):
        if self.plane == 'coronal':
            self._model.update_section(
                SetPosition(axis=Axis.Horizontal, value=x),
                SetPosition(axis=Axis.Longitudinal, value=y)
            )
        elif self.plane == 'axial':
            self._model.update_section(
                SetPosition(axis=Axis.Horizontal, value=x),
                SetPosition(axis=Axis.Anteroposterior, value=y)
            )
        elif self.plane == 'sagittal':
            self._model.update_section(
                SetPosition(axis=Axis.Anteroposterior, value=x),
                SetPosition(axis=Axis.Longitudinal, value=y)
            )
