...
@pytest.fixture
def commands(model, atlas_volume):
...
@pytest.fixture
def sidebar_view(model, commands):
    return SidebarViewModel(_model=model, _commands=commands)


@pytest.fixture
def volume_view(model, commands):
    return VolumeViewModel(_model=model, _commands=commands)
